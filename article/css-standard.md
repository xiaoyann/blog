# 我的CSS规范  
对于一个有轻微强迫症的我来说，写CSS时命名真的很痛苦。有时候因为想不出好的命名要花掉好几分钟。严重影响工作效率。因此整理出一套CSS规范势在必行。
...
### 命名规范
> * 描述出元素是什么、作什么用、怎么样。
> * 外层的容器，按作用描述，按属性描述，因为这时你叫它啥都不合适
> * 参考BEM(模块-元素-修饰), 但不是完全遵守
> * 较长的单词按发音首字母缩写，而不是全拼
> * 全站通用的加site前缀
> * 常用：bd hd bt ft box 
